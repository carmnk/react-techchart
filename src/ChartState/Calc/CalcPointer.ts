...
export const calculatePointer = (
  ...
  const { subcharts, data } = ChartState;
  const mainGraph =
  ...
  const clickedSubchartIdx = !!dragEnd && !!dragEnd.xy ? getSubchartIdxByPixXy(dragEnd.xy, subcharts) : null;
  const snapDatasets = subcharts[subchartIdx].yaxis
    .map((yaxis, yaxisIdx) =>
      yaxis.graphs.map((graph, graphIdx) => {
        const graphDataSeries = data.find((val) => val.id === graph?.dataId)?.data as T.DataSeries;
        const snapPriceRes = graphDataSeries
          ? snapPixYToDataset(xy[1], graphDataSeries[x], subcharts, subchartIdx, yaxisIdx, calc.subcharts)
          : null;
        const pixYSnap = snapPriceRes && snapPriceRes.length > 0 ? snapPriceRes[0].pixY : null;
        const ySnap = snapPriceRes && snapPriceRes.length > 0 ? snapPriceRes[0].y : null;
        const { decimals, translatedY, heightPerPt } = calc.subcharts[subchartIdx].yaxis[yaxisIdx];
        return {
          yaxisIdx: yaxisIdx,
          graphIdx: graphIdx,
...
const calculatePointerXvals = (
  ...
  const { scaledWidthPerTick: widthPerTick, totalTranslatedX: translatedPixX } = xaxis;
  const xUnlimited = Math.round(purePixToX(onMove?.xy[0], translatedPixX, widthPerTick));
  ...
  if (!optChartPeriod || !mainGraphMeta.chartPeriod || !mainGraphDateStat) return { xUnlimited, pixXUnlimSnap };
  const periodToDraw = getTickPeriod(mainData.data[x].date, mainGraphDateStat, mainGraphMeta.chartPeriod, {
    ...optChartPeriod,
  });
