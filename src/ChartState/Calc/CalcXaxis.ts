...
const scaleFn2 = (
  deltaPixX: number,
  scaleInitWidthPerTick: number,
  containerWidth: number
) => {
  return deltaPixX >= 0
    ? Math.min(
        scaleInitWidthPerTick *
          (1 + Math.abs(deltaPixX) / (containerWidth / 2)),
        containerWidth / 3
      )
    : Math.max(
        scaleInitWidthPerTick /
...
const getOptimalPeriod = (
  ...
  const { name: chartPeriodName, multiply: chartPeriodMultiply } =
    metaData.chartPeriod;
  const periodsConstIdx = chartPeriods.findIndex(
    (periodConst) => periodConst.name === chartPeriodName
  );
  ...
  const optPeriodRes = chartPeriods
    ...
    .map((constPeriod, constIdx) => {
      ...
      const multiplys =
        constIdx === 0
          ...
          : [1, ...constPeriod.scaleMultiplys];
      return [1, ...multiplys].map((multiply, mIdx) => {
    ...
    .flat()
    .reduce((acc, cur, idx) =>
      minimizationFunction(cur.amt, cur.multiply) <
...
const calcXaxisVals = (
  ...
  const xStartExaxt = !scaledWidthPerTick
    ? 0
  ...
  const mainGraphData = data.find(
    (val) => val.id === mainGraph?.dataId
  ) as T.ChartData;
  if (
    !containerWidth ||
    !mainGraph ||
  ...
  const pixXEnd = Math.min(
    containerWidth,
    nMainData * scaledWidthPerTick + translatedPixX
  );
  const pixXEndUnlimited = containerWidth;
  const xUnlimited = Math.floor(
    (pixXEndUnlimited - translatedPixX) / scaledWidthPerTick
  );
...
export const calculateXaxis = (
  ...
  const doDragAction = action?.drag?.shallUpdate;
  const doTranslate = action?.drag?.type === "translate" && doDragAction;
  const doScale = action?.drag?.type === "scale" && doDragAction;
  const mainGraphData = data.find((val) => val.id === mainGraph?.dataId);
  const dragInit = pointer.drag?.initial;
  if (
    !mainGraphData ||
    (!action.drag?.shallUpdate &&
      !action.wheel &&
      !action.deps &&
      !action.containerResize) ||
    (!dragInit && action.drag?.shallUpdate)
  )
    return xaxis;
  const nMainData = mainGraphData.data.length;

  if (isPinching) {
    const pinchAction = action.pinch as NonNullable<T.Action["pinch"]>;
    ...
    const scaledWidthPerTick = scaleFn2(
      pointer?.pinch?.movementInitial[0],
      initScaledWidthPerTick,
      canvasWidth
    );
    const translatedXonStart = initTranslatedX;
    const initPixX = dragInit[0];
    const initXexact =
      (dragInit[0] - translatedXonStart) / initScaledWidthPerTick;
    ...
    const initXexact =
      ((initWheelMousePos ? initWheelMousePos[0] : 0) - translatedXonStart) /
      (xaxis.scaledWidthPerTick as number);
    const newPos =
      Math.round(initXexact) * scaledWidthPerTick + xaxis.totalTranslatedX;
    ...
    const totalTranslatedX = Math.max(
      Math.min(
        pointer.drag.delta[0] + xaxis.totalTranslatedX,
        canvasWidth - 2 * xaxis.scaledWidthPerTick
      ),
    ...
    const initPixX = dragInit[0];
    const initXexact =
      (dragInit[0] - translatedXonStart) / (initScaledWidthPerTick as number);
    const newPos = initXexact * scaledWidthPerTick + translatedXonStart;
    const totalTranslatedX = initTranslatedX - newPos + initPixX;
...
export const jumpToXaxisEnd = (
  ...
  return {
    ...xaxis,
    totalTranslatedX:
      -(mainGraphData.length + 10 - containerWidth / xaxis.scaledWidthPerTick) *
      xaxis.scaledWidthPerTick,
  };
};
