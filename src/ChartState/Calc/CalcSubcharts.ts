      ...
      yaxis: subchart.yaxis.map((oneYaxis, yIdx) => {
        ...
        const yMaxExact =
          Math.max(
            ...
            ...([{ data: rtData, dataId: maingraphId }]
              ?.filter?.((val) => oneYaxis.graphs.map((g) => g.dataId).includes(val.dataId))
              .map((rtDat) =>
